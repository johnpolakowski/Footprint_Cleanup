...
class FileHelper:
    ...
    def Get_XML_Files(directory_path):
        ...
        # iterate through each .xml file in current directory
    ...
    def Get_STEP_Files(directory_path):
        ...
        print(directory_path)
        # iterate through each .xml file in current directory
        for filename in glob.glob(os.path.join(directory_path, '*.step')):
            file_list.append(filename)
            print("\t" + FileHelper.Get_Base_Filename(filename))
        return file_list
    ...
    def Rename_Files(directory_path, orig_extension, new_extension):
        ...
        # iterate through each .xml file in current directory
        for filename in glob.glob(os.path.join(directory_path, search_extension)):
            file_list.append(filename)

        for old_file in file_list:
            old_name_no_extension = FileHelper.Get_Filepath_No_Extension(
                old_file)
    ...
    def Get_Filename_No_Extension(filepath):
        # get path of xml file without the ".xml" extension
        read_file_no_ext = os.path.splitext(filepath)[0]
        # replace backslashes with forward slashes
        file_no_ext_fixed = read_file_no_ext.replace(os.sep, '/')
    ...
    def Match_STEP_Files(XML_dir, STEP_files, Allegro_STEP_dir):
        # get path of xml file without the ".xml" extension
        read_file_no_ext = os.path.splitext(Allegro_STEP_dir)[0]
        # replace backslashes with forward slashes
        file_no_ext_fixed = read_file_no_ext.replace(os.sep, '/')
        return file_no_ext_fixed
    ...
    def Wait_For_File_Download(files_before, download_dir):
        ...
        while 1:
            ...
            new_files = [
                file for file in files_after if not file in files_before]
            if len(new_files) > 0:
