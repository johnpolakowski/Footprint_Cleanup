...
class Footprint_Parser:
    ...
    def Change_Layer(self, initial_layer, destination_layer):
        if self.root is None:
            return None

        layer_exists = self.Check_Layer_Present(initial_layer)    
        if layer_exists:
            print("changing'{}' to '{}'\n".format(initial_layer, destination_layer))
            if self.root is None:
                return None
            layer_block = self.root.find('Layers')
            all_layers = layer_block.findall('Layer')
            for layer in all_layers:
                layer_name = layer.get('name')
                if layer_name == initial_layer:
                    layer.set('name', destination_layer)
                    print("\tchanged Layer name: ", layer_name)
