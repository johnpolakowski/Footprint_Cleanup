...
# Add pin 1 indicator
    # pin1 ind defined by arc
    # check for filled arcs on package geo ass top or package geo silk top
    # create bounding box of footprint pads
    # place pin1 indicators in center of pin outside bounding box
...
# if ultralibrarian:
#delete date directory with step file
# remove least and most xml files
# remove default builder
# remove import guide in all dirs

XML_create_directory = './done/'
XML_search_path = './examples/'

def Process_XML_Files(XML_files):

    if not FileHelper.Dir_Exists(XML_create_directory):
        FileHelper.Create_Dir(XML_create_directory)
    ...
    for file in XML_files:
        with open(file, 'r') as footprint_xml:
            ...
            parser.Change_Layer('MANUFACTURING/NO_PROBE_TOP', 'PACKAGE GEOMETRY/DFA_BOUND_TOP')
            # removes extra crap thrown in footprints
            parser.Remove_Undesired_Layers()
